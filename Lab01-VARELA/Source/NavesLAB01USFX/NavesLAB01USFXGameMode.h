...
// GameMode del juego que hereda de AGameModeBase de Unreal, es el que controla todo como: spawna las naves, maneja los timers y decide cuándo formar o romper la posición
UCLASS(MinimalAPI)
...
public:
    ...
    FTimerHandle TimerFormacion;           // Timer que intenta formar cada 5 segundos
    FTimerHandle TimerVerificarMovimiento; // Timer que revisa el estado cada 0.5 segundos

    void FormarNavesFrentePawn();      // Calcula las posiciones y se las manda a cada nave
    ...
    APawn* PlayerPawn; // Referencia al jugador para calcular dónde va el centro 

    float IntervaloFormacion = 5.0f;  // Cada cuántos segundos se intenta activar la formación

    float EspaciadoVertical = 100.0f; // Tamaño general de la formación
