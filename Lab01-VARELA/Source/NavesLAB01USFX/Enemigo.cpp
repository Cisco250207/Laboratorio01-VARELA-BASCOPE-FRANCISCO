...
AEnemigo::AEnemigo()
{
    ...
    // Buscar la malla Shape_Cube en el StarterContent y asignarla automáticamente
...
void AEnemigo::Tick(float DeltaTime)
{
    ...
    // Modo 1: formación activa, va directo a su posición 
    if (bEnFormacion)
    {
        ...
        {
            // Avanzar en línea recta hacia su posición 
...
// El GameMode llama esto para activar la formación y decirle dónde es el centro 
void AEnemigo::ActivarFormacion(FVector Centro, float Escala)
{
    CentroFormacion = Centro; // Guardar el centro  en el mundo
...
// Calcula la posición exacta en el mundo que le toca a este enemigo  
// Usa una tabla de offsets relativos al centro
FVector AEnemigo::ObtenerPosicionEnFormacion()
{
    // Cada offset es la posición relativa 
...
TArray<FVector> Offsets = {
    ...
    // Si el índice no existe por alguna razón, ir al centro 
    if (!Offsets.IsValidIndex(IndiceFormacion))
    {
        return CentroFormacion;
    }

    // Posición final = centro + offset de su parte multiplicado por la escala
