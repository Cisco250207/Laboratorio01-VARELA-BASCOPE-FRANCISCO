...
public:
    ...
    UPROPERTY(EditAnywhere, Category = "Formacion")
        int32 IndiceFormacion = 0; // Número del 0 al 9 que dice qué parte que le toca a este enemigo

    UPROPERTY(EditAnywhere, Category = "Formacion")
        FVector CentroFormacion = FVector(0, 0, 200); // Punto central, los offsets se calculan desde aquí

    UPROPERTY(EditAnywhere, Category = "Formacion")
        bool bEnFormacion = false; // true = va a su posición, false = vaga libre por el nivel

    UPROPERTY(EditAnywhere, Category = "Formacion")
        float EscalaFormacion = 250.0f; // Tamaño de la formación, a mayor valor más separadas las naves
...
private:
    ...
    FVector ObtenerPosicionEnFormacion(); // Calcula dónde exactamente debe ir según su índice 
};